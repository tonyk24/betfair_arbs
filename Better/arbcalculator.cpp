...
int ArbCalculator::m_currentArbBetId = 0;
...
void ArbCalculator::chooseBets(const BfLib::BfEvent &event,
                               const std::list<BfLib::BetfairMarket> &markets,
                               std::list<BfArbBet> &currentArbBets,
                               double maxExposure,
                               EventStats *m_eventStats) {
  std::list<TwoCombination> twoCombinations;
  findTwoCombinations(m_eventStats,event, markets, twoCombinations);
  ...
  for (std::list<TwoCombination>::iterator it = twoCombinations.begin();
    ...
    if ( it->m_type == TwoCombination::BACKNG_NO_GOAL_to_BACKOU05_OVER ||
      ...
      if (hasData(backFirst, backSecond)) {
        ...
        if (p1+p2 < 0.97) {
          qDebug() << "Found arbitrage bet back/back " << event.m_eventId << " {" << it->market1->m_marketName << "," << it->selection1->m_selectionName
          ...
          qDebug() << "impliedMaxExposure1= " << impliedMaxExposure1;
          qDebug() << "impliedMaxExposure2= " << impliedMaxExposure2;
          ...
          qDebug() << "(takePrice1=" << QString::number(takePrice1) << ", takeSize1= " << QString::number(takeSize1) << ", takePrice2= " << QString::number(takePrice2) << ",takeSize2=" << QString::number(takeSize2);
          qDebug() << "Outcome1: " << QString::number((takePrice1-1)*takeSize1 - takeSize2);
          qDebug() << "Outcome2: " << QString::number(-takeSize1 + (takePrice2-1)*takeSize2);
          ...
          if (takeSize1 >= std::min(30.0, backFirst.second) &&
            ...
            //placeBets[it->market1->m_marketId].push_back(bet1);
            //placeBets[it->market2->m_marketId].push_back(bet2);
            m_currentArbBetId++;
            currentArbBets.push_back(BfArbBet(m_currentArbBetId,
                                              *(it->market1),
                                              *(it->selection1),
                                              bet1));
            currentArbBets.push_back(BfArbBet(m_currentArbBetId,
      ...
      if (hasData(backFirst, laySecond)) {
        ...
        if (p1+p2_lay < 0.97) {
          ...
          qDebug() << "Found arbitrage bet back/lay " << event.m_eventId << "{" << it->market1->m_marketName << "," << it->selection1->m_selectionName
                   << "} {" << it->market2->m_marketName << "," << it->selection2->m_selectionName << "}"
                   << "(price= " << backFirst.first << ", size= " << backFirst.second << ") ("
          ...
          qDebug() << "(takePrice1=" << QString::number(takePrice1) << ", takeSize1= " << QString::number(takeSize1) << ", takePrice2= " << QString::number(takePrice2) << ",takeSize2=" << QString::number(takeSize2);
          qDebug() << "Outcome1: " << QString::number((takePrice1-1.0)*takeSize1 - (takePrice2-1.0)*takeSize2);
          qDebug() << "Outcome2: " << QString::number(-takeSize1+takeSize2);
          ...
          if (takeSize1 >= std::min(30.0, backFirst.second) &&
            ...
            if (takeSize1 < std::min(30.0, backFirst.second) &&
              ...
              qDebug() << "Outcome1: " << QString::number(outcome1);
              qDebug() << "Outcome2: " << QString::number(outcome2);
...
*/
      ...
      if (hasData(layFirst, laySecond)) {
        ...
        if (p1_lay+p2_lay < 0.97) {
          qDebug() << "Found arbitrage bet lay/lay " << event.m_eventId << " {" << it->market1->m_marketName << "," << it->selection1->m_selectionName
                   << "} {" << it->market2->m_marketName << "," << it->selection2->m_selectionName << "}"
...
void ArbCalculator::findTwoCombinations(EventStats *m_eventStats,
                                        const BfEvent &event,
